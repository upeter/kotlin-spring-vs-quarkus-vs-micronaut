...
class FruitControllerTest {
    ...
    fun `should crud entity`() {
        ...
        val updated = blockingClient.retrieveObject<Fruit>(HttpRequest.PUT("/fruits/${created.id}", Fruit(created.id, "Peach")))
        created.id shouldBe updated.id
        blockingClient.retrieveObject<Fruit>(HttpRequest.GET("/fruits/${created.id}")) shouldBe updated
        blockingClient.retrieveList<Fruit>(HttpRequest.GET("/fruits")).size shouldBe 1
...
//val updated = blockingClient.retrieveObject<Fruit>(HttpRequest.PUT("/fruits/${created.id}", Fruit(created.id, "Apple")))
